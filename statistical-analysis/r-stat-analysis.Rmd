...
install.packages("RColorBrewer") # a color palette package
```
...
library(RColorBrewer) # a color palette package
```
...
Rub a simple linear regression
```{r}
...
```{r}
scores %>%
  ggpairs(ggplot2::aes(fill=gender))+
  scale_fill_brewer(palette = "Dark2")
```
