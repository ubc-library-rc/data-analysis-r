...
author: "Albina Gibadullina, Maria Laricheva and Ashley Yue Mao"
date: "November 26, 2021 | created
...
This workshop uses the following R packages (what is a package?): 
...
Using the help function to look at description of the in-built dataset
```{r}
?sat.act
```
...
```{r}
class(scores$gender) 
# Note. The dollar sign $  lets you access variables within a data set
```
...
# Note. The square bracket  [] lets you subset an object based on certain condition or index.
...
By default, the levels are ordered alphabetically. So when we simply force gender to be a factor, men will be the first level, and women will be the second level. If the order you want does not match with the alphabetic order, here is how you can reorder levels. This is useful when you want certain category of a categorical variable to be the reference group (i.e., make that group the first level).
```{r}
scores$gender <- forcats::fct_relevel(scores$gender, c("Men", "Women"))
levels(scores$gender)
```
...
How many of you are familiar with piping `%>%`? Please give me a yes or no reaction.

Pip takes the results of the line before it and feed it to the line after it. The advantage of using piping is to avoid nested parentheses. Here is how nested parentheses look like
```{r}
get_summary_stats(group_by(scores, gender), ACT)
```
It it easier to write, and easier to read as well if someone need to review your code.


Generated a summary statistics table, grouped by gender (using `table1` package)
```{r}
...
Visualize the distribution of ACT scores using a histogram with a normal distribution curve and a line for mean (refer to our `Visualization with R workshop` for more details of the using `ggplot2`)
```{r}
scores %>%
  ggplot(aes(x = ACT)) + 
  ...
  stat_function(fun = dnorm, 
                args = list(mean = mean(scores$ACT), 
                            sd = sd(scores$ACT)), size = 1) +
  geom_vline(aes(xintercept = mean(ACT)), color = "red") +
  geom_text(aes(x = mean(ACT), label = round(mean(ACT), 0), y = -0.005), 
            colour = "red", size = 4)
...
scores %>% 
  ggplot(aes(fill = gender, x = gender)) + 
    ...
    geom_text(aes(label = round(stat(count)/sum(count),2), 
    ...
    labs(title = "Share of observations by gender",
         x = "Gender", y = "Percent of observations", 
         fill = "Gender")
```
...
scores %>%
  ggplot(aes(x = gender, y = SATV, fill = gender)) +
    ...
    stat_summary(fun = mean, geom = "point", 
                 shape = 5, size = 4) +
    scale_fill_brewer(palette = "Dark2") +
    labs(title = "Boxplots of SAT Verbal scores by gender",
         x = "Gender", y = "SAT Verbal scores",
         fill = "Gender")
```
...
scores %>%
  ggplot(aes(x = education, y = SATV, fill = education)) +
    ...
    labs(title = "Boxplots of SAT Verbal scores by education level",
         x = "Levels of education", y = "SAT Verbal scores", 
         fill = "Education")
```
...
scores %>%
  ggplot(aes(x = education, y = SATV, fill = gender)) +
    ...
    labs(title = "Boxplots of SAT Verbal scores by education level and gender",
         x = "Level of education", y = "SAT Verbal score", 
         fill = "Gender")
```
...
scores %>% 
  ggplot(aes(fill = gender, x = education)) + 
    ...
    labs(title = "Distribution of observations by gender and education levels",
         x = "Education levels", y = "Number of observations", 
         fill = "Gender") +
    coord_flip()
```
...
X-squared(df=5) = 16.085, p-value = 0.006% < 5% (reject the null) -> Gender and education levels are dependent. As gender change, the distribution of educational level change, vice verse.
...
scores %>%
  ggpairs(aes(fill = gender, color = gender, alpha = 0.3), 
          progress = F) +
  scale_fill_brewer(palette = "Dark2") +
...
Does any one know what nested model?
...
P-values below 5% indicate that the new model is an improvement over the previous one:
 ...
 - Model 3 is an improvement over Model 2 
...
plot=plot(model3, 3)
```
If the constant variance assumption hold, the residuals roughly form a "horizontal band" around the 0 line.
...
plot=plot(model3, 1)
```
If the linearity assumption hold, the residuals "bounce randomly" around the 0 line.
...
While assumptions of linear model are never perfectly met, we must still check if they are reasonable assumptions to work with. When these assumptions are severely violated, you may consider deleting or transforming certain variables (violation of multicollinearity, normality, linearity), switching to nonlinear model (violation of linearity) or multilevel model (violation of independence), etc.
